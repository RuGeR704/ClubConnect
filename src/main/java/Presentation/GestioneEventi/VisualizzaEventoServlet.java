...
public class VisualizzaEventoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (utente != null) {
                // Controllo se l'utente partecipa già
                isPartecipante = service.isUtentePartecipante(utente.getId_utente(), idEvento);

                // Controllo se è admin/gestore del gruppo (per tasto elimina)
                if (gruppo != null) {
                    isAdmin = service.isUtenteGestore(utente.getId_utente(), gruppo.getId_gruppo());
                }
            }
