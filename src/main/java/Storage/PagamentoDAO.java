...
// L'ordine dei ? è: 1:id_utente, 2:nome, 3:cognome, 4:numero, 5:scadenza, 6:WHERE id_metodo
     ...
     */
    public boolean isAbbonamentoValido(Connection con, int idUtente, int idGruppo) throws SQLException {
        // Query semplificata: Controlla solo se esiste un pagamento.
        // "SELECT 1" è molto veloce perché non deve recuperare dati, solo verificare l'esistenza.
        String query = "SELECT 1 FROM Pagamento p " +
        ...
        try (PreparedStatement ps = con.prepareStatement(query)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                // Se rs.next() è true, significa che ha trovato un pagamento.
                // Quindi l'utente "ha pagato" e non deve pagare di nuovo.
                return rs.next();
            }
        }
    }
